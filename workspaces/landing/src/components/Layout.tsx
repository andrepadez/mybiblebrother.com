    ...
    <div className="min-h-screen w-full bg-bible-light overflow-x-hidden">
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-bible-skyblue/70 text-white py-3 px-4 shadow-md z-50 backdrop-blur-sm">
        <div className="container mx-auto flex justify-between items-center">
          <div className="font-serif text-xl">My Bible Pal</div>
          ...
          <nav className="flex space-x-6">
            ...
            <Link
              to="/chat"
              className={`flex items-center gap-1 transition-colors ${location.pathname === '/chat' ? 'text-white' : 'text-white/70 hover:text-white'}`}
            >
              <MessageSquare size={18} />
              <span className="hidden sm:inline">Bible Pal</span>
            </Link>
          </nav>

          <div className="text-xs opacity-80 hidden sm:block">
            &copy; {new Date().getFullYear()} My Bible Pal
          </div>
        </div>
      </footer>
    </div>
